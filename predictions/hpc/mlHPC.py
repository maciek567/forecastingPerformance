...
class ReservoirHPC(PredictionHPC):
    ...
    def extrapolate(self, params: dict) -> ndarray:
        train = dl.loader_explicit(utilsHPC.normalize(self.data_to_learn, self.data_to_learn), test_size=0)
        _, x_train, _, y_train = train()
        test = dl.loader_explicit(utilsHPC.normalize(self.data_to_validate, self.data_to_learn), test_size=0)
        ...
        return utilsHPC.denormalize(np.array(result), self.data_to_learn)

    def plot_extrapolation(self, result) -> None:
        utilsHPC.plot_extrapolation(self, result, PredictionMethod.Reservoir)


class XGBoostHPC(PredictionHPC):
    def __init__(self, prices: Series, real_prices: Series, prediction_border: int, prediction_delay: int,
                 column: SeriesColumn, deviation: DeviationSource, mitigation_time: int = 0):
        super().__init__(prices, real_prices, prediction_border, prediction_delay, column, deviation, mitigation_time)
